...
const AdminUsersListPage: React.FC = () => {
    ...
    const primaryButtonClasses = `flex items-center gap-2 rounded-lg bg-${accentColor}-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-${accentColor}-500/30 transition-all duration-300 hover:bg-${accentColor}-400`;
    ...
    return (
        <div>
            ...
            <header className="mb-8 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h1 className="text-3xl sm:text-4xl font-bold tracking-tight">مدیریت کاربران</h1>
                            ...
                            {users.map(user => (
                                <tr key={user.email} className="border-b dark:border-gray-700 hover:bg-black/5 dark:hover:bg-white/5 transition-colors">
                                    ...
                                    <td className="px-6 py-4 text-left">
                                        <Link to={`/admin/users/${encodeURIComponent(user.email)}`} className={`font-medium text-${accentColor}-600 dark:text-${accentColor}-500 hover:underline`}>
                                            مدیریت
                                        </Link>
                                    </td>
                                </tr>
            ...
            <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="افزودن کاربر جدید">
                <form onSubmit={handleAddUser} className="space-y-4">
                    ...
                    <div className="flex justify-end pt-2">
                        <button type="submit" className={`rounded-lg bg-${accentColor}-500 px-4 py-2 text-sm font-semibold text-white shadow-lg shadow-${accentColor}-500/30 transition-all duration-300 hover:bg-${accentColor}-400`}>ایجاد کاربر</button>
                    </div>
                </form>
            </Modal>
        </div>
